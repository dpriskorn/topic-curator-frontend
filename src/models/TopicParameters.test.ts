...
describe("SparqlItem", () => {
  it("should initialize with default values", () => {
    const term = new Term("test", TermSource.LABEL);
  ...
  it("should allow custom initialization", () => {
    const term = new Term("science", TermSource.ALIAS);
    const item = new SparqlItem({
      qid: "Q42",
  ...
  it("should highlight term in itemLabel", () => {
    const term = new Term("Science", TermSource.LABEL);
    const item = new SparqlItem({
      qid: "Q42",
  ...
  it("should not modify itemLabel if no match is found", () => {
    ...
    expect(item.highlightedItemLabel).toBe("Quantum Science and Technology");
  });
  ...
  it("should throw an error if term is not a string", () => {
    expect(() => new Term(123 as unknown as string, TermSource.LABEL)).toThrow(
      "Expected string for term, but got number, 123"
    );
  });
});
